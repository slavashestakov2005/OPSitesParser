...
class ATask:
	def __init__(self, id, good, bad, lang_translate):
...
class ATasks:
	...
	def writable(self, last_solve):
		...
		for part in tasks:
			...
			good = sorted(t[0], key=ATask.sortf)
			bad = sorted(t[1], key=ATask.sortf)
...
def parse_user_profile(userId):
	...
	# print(userName)
	return AUser(userId, userName, rank, rating)
...
def parse_user_submissions(userId, raw_file, lang_translate):
...
def parse_group(cache_folder, group_folder, users, lang_translate):
	...
	for now in users:
		...
		tasks, user.send, last_solve = parse_user_submissions(now, cache_raw_folder + str(user.id) + '.txt', lang_translate)
